...
do
	...
	local function SetDisplayCols (self, cols)
		...
		for i = 1, #cols do
			...
			-- Mebner corrected this behaviour!
			if type(cols[i].defaultsort) == "string" then
				if cols[i].defaultsort:lower() == "asc" then cols[i].defaultsort = lib.SORT_ASC;
				elseif cols[i].defaultsort:lower() == "dsc" then cols[i].defaultsort = lib.SORT_DSC;
				else cols[i].defaultsort = nil; end
			end
			if type(cols[i].sort) == "string" then
				if cols[i].sort:lower() == "asc" then cols[i].sort = lib.SORT_ASC;
				elseif cols[i].sort:lower() == "dsc" then cols[i].sort = lib.SORT_DSC;
